...
OUT_DIR="${OUT_DIR:-${GEAK_ROOT}/optimization_logs/mhc_head_cu_hygon_opt_from_repo}"
...
export MHC_HEAD_FORCE_REBUILD="${MHC_HEAD_FORCE_REBUILD:-1}"
...
geak --config "${GEAK_CONFIG}" \
  ...
  --kernel-url "${REPO_ROOT}/examples/mhc_ops/src/mhc_head.cu" \
  --test-command "python3 ${REPO_ROOT}/examples/mhc_ops/test_mhc_head_hip_harness.py --full-benchmark" \
  --task "Optimize the CUDA/HIP hc_head baseline kernel at ${REPO_ROOT}/examples/mhc_ops/src/mhc_head.cu for Hygon K500SM_AI DCU gfx928. The source is compiled by hipcc with -x hip. Metric is GEAK_RESULT_LATENCY_MS, lower is better. Preserve the formula hidden = sum_c head_mix[c] * residual[c], FP32 accumulation, and BF16 output correctness for all harness shapes." \
  --gpu-ids 0 \
  --num-parallel 1 \
  --debug \
  --yolo \
